  ...
  <div class="m-2 mt-4">
    <label class="mdc-text-field mdc-text-field--outlined" ref="mainRef">
      ...
      <datalist :id="`textfield-list-${id}`">
        <option v-for="option of autocompleteOptions" :data-value="option?.id">{{ option?.label }}</option>
      </datalist>
    </label>
...
const autocompleteOptions = computed(() => (props.options ?? []) as {
  id: number,
  label: string,
}[])
const optionsIdsByLabels = computed(() => (autocompleteOptions.value.reduce((acc: { [key: string]: number }, curr) => {
  acc[curr.label] = curr.id
  return acc
}, {})))
const optionsLabelsByIds = computed(() => (autocompleteOptions.value.reduce((acc: { [key: number]: string }, curr) => {
...
function input(event: Event) {
  ...
  let potentialValue = optionsIdsByLabels.value[value] ?? null
    ...
    to => {
      setTimeout(() => {
        if (typeof to == 'number' && mdcTextfield.value) {
          mdcTextfield.value.value = optionsLabelsByIds.value[Number(to)]
        }
      })
    },
    { immediate: true }
